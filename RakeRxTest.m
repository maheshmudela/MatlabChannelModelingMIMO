...
dschBitsSymbolMapped = randi([-15 16], 1, length(dschBitsSymbol) + j*randi([-15 16], 1, length(dschBitsSymbol)
...
orthogonalData  =zeros(1, length(DataSymbolPerSecond)*length(W1));
orthogonalPilot =zeros(1, length(DataSymbolPerSecond)*length(W0));
...
DschSymbolPerSlot = 24;

orthogonalDataPerFrame20=[DataSymbolPerSecond(1:DataSymbolPerSlot*18*50)];
pilotSymPerFrame20ms=[pilotSymPerSec(1:(PilotSymbolPerSlot*50*18))];

DschDataPerFrame20 = [DataSymbolPerSecond(1:DataSymbolPerSlot*18*50)];
...
for slotIndex=1:16
    ...
    Dsch  = [dschBitsSymbolMapped((slotIndex-1)*DschSymbolPerSlot+1:slotIndex*DschSymbolPerSlot)];

    DschSpread = kron(Dsch, W2);
    ...
    pilotSymbolPerSlotScrambled = (LongCodeComplexP+pilotSymbolPerSlotSpread)%2;
    ...
    DataSymbolPerSlotSpread = kron(DataSymbolPerSlotRepeat,W1)
    ...
    ILongCodeComplexD = (ILongCodeUser + IchannelShortCode)%2;
    ...
    QLongCodeComplexD = (QLongCodeUser  + QchannelShortCode)%2;

    QLongCodeComplexD_Dec2   = [QLongCodeComplexD(1:2:end)]

    %Walsh spread by 2;
    QLongCodeComplexD_Spread = kron(QLongCodeComplexD_Dec2,H1(2);
    ...
    DschScrambled = Dsch.*LongCodeComplexD;
    ...
    [Idata1  Qdata1] = [real(FinalSlotDataPunctured) imag(FinalSlotDataPunctured)];
    [1data2  Qdata]  = [real(DschScrambled)  imag(DschScrambled)];

    Idata = Idata1 + 1data2; %Orthogonally added
    Qdata =Qdata1 + Qdata2; %Orthogonally added
...
% Rake receiver for traffic after channel estimation
for slotIndex=1:16
   descsrambledI = = real(RxCorrectedIQ((slotIndex-1)+1:slotIndex*1556) .*real(pnQuadLongSeqQ)); %Tc/Ts=N
   for pathL=1:L

      % Channel estimate is aftre decsraambling
      equalizedSymbol(l) = descsrambledI(pathL:1536-pathL) *.(1/hi(pathL);
